...
public class ReservationService {
	...
	public List<Reservation> getReservationsByGuestId(Long guestId){
		...
		List<Reservation> reservationByGuestId=new ArrayList<>();
		GuestDTO g2 = guestFeign.getGuestById(guestId);
		
		for (Reservation reservation:allReservations) {
			if (g2.getEmail().equals(reservation.getEmail())) {
				reservationByGuestId.add(reservation);
			}
		}
		
		return reservationByGuestId;
	}
